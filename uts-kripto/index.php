...
class KriptoTools {
    ...
    public static function getRSAConfig() {
        ...
        // PENYESUAIAN UNTUK INFINITYFREE & LOCALHOST
        // Jika file openssl.cnf ada (di XAMPP), gunakan path tersebut.
        // Jika tidak ada (di InfinityFree/Linux), abaikan dan gunakan bawaan server.
        $confPaths = array(
...
if (isset($_POST['generate_keys'])) {
    ...
    if ($keys) {
        ...
        $error = "Gagal membuat kunci RSA. Fungsi openssl_pkey_new() mungkin diblokir oleh pihak Hosting.";
    }
    $activeTab = 'rsa';
}

// Proses Eksekusi Kriptografi
    ...
    try {
        if ($activeTab === 'caesar') {
            ...
            if ($action === 'encrypt') {
                ...
                if ($res) $result = $res;
                else $error = "Gagal mengenkripsi! Pastikan Public Key valid.";
            } elseif ($action === 'decrypt') {
                $res = KriptoTools::rsaDecrypt($textInput, $rsaPrivateKey);
                if ($res) $result = $res;
                else $error = "Gagal mendekripsi! Pastikan Private Key benar dan format Base64 valid.";
            }
...
<head>
        ...
        tailwind.config = {
            theme: {
                extend: {
                    ...
                    colors: {
                        gray: { 850: '#1f2937', 900: '#111827', 950: '#0a0a0f' },
    ...
    <style>
        ...
        .nav-item.active { background: rgba(99, 102, 241, 0.1); color: #818cf8; border-right: 3px solid #6366f1; }
    ...
    <aside class="w-72 flex-shrink-0 flex flex-col border-r border-gray-800/80 bg-gray-900/60 backdrop-blur-2xl z-20 hidden md:flex transition-all">
        ...
        <div class="p-6 border-t border-gray-800/80 bg-gray-950/40">
            <div class="flex items-center gap-3 text-xs text-gray-500 font-mono">
                <i class="ph-duotone ph-code text-lg text-brand-400/70"></i> Live on InfinityFree
            </div>
        </div>
    </aside>
                ...
                <div class="glass-card rounded-2xl shadow-2xl shadow-black/50 overflow-hidden relative border-t border-t-gray-700/50">
                    <form method="POST" action="" class="p-6 md:p-8 space-y-8 relative z-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <?php if ($activeTab === 'caesar'): ?>
                                <div>
                                    ...
                                    <input type="number" name="shift" value="<?= htmlspecialchars($shift) ?>" class="w-full bg-gray-900/80 border border-gray-700/80 rounded-lg px-4 py-3.5 text-white font-mono text-sm focus:border-brand-500 focus:bg-gray-900 outline-none transition-all shadow-inner" required>
                                </div>
                            <?php endif; ?>
                            <?php if ($activeTab === 'xor'): ?>
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-medium text-gray-400 mb-2">Symmetric Key</label>
                                    <input type="text" name="xor_key" value="<?= htmlspecialchars($xorKey) ?>" class="w-full bg-gray-900/80 border border-gray-700/80 rounded-lg px-4 py-3.5 text-white font-mono text-sm focus:border-brand-500 focus:bg-gray-900 outline-none transition-all shadow-inner" placeholder="Kata sandi rahasia..." required>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <?php if ($activeTab === 'rsa' || $activeTab === 'signature'): ?>
                            <div class="pt-2 border-t border-gray-800/80">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="text-sm font-medium text-gray-300 uppercase tracking-widest">Kunci Asimetris (PEM)</h3>
                                    <button type="submit" name="generate_keys" value="1" formnovalidate class="text-[11px] uppercase tracking-widest font-bold bg-gray-800 hover:bg-gray-700 text-yellow-500 border border-yellow-500/30 rounded py-1.5 px-3 transition-colors flex items-center gap-1.5">
                                        <i class="ph-bold ph-lightning"></i> Buat Kunci
                                    </button>
                                </div>
                                <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-400 mb-2">PUBLIC KEY</label>
                                        <textarea name="rsa_public_key" spellcheck="false" rows="5" class="w-full bg-gray-950 border border-gray-800 rounded-lg p-3 text-gray-400 font-mono text-[11px] leading-relaxed focus:border-brand-500 outline-none block custom-scrollbar resize-y shadow-inner"><?= htmlspecialchars($rsaPublicKey) ?></textarea>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-400 mb-2">PRIVATE KEY</label>
                                        <textarea name="rsa_private_key" spellcheck="false" rows="5" class="w-full bg-gray-950 border border-gray-800 rounded-lg p-3 text-gray-400 font-mono text-[11px] leading-relaxed focus:border-brand-500 outline-none block custom-scrollbar resize-y shadow-inner"><?= htmlspecialchars($rsaPrivateKey) ?></textarea>
                                    </div>
                                </div>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php if ($activeTab === 'signature'): ?>
                            <div class="pt-4">
                                <label class="block text-sm font-semibold tracking-wide text-gray-300 mb-2 uppercase">Signature Hash (Base64)</label>
                                <textarea name="signature_input" rows="2" class="w-full bg-gray-900/80 border border-gray-700/80 rounded-xl p-4 text-pink-400 font-mono text-sm leading-relaxed focus:border-pink-500 outline-none resize-y placeholder-gray-600 custom-scrollbar shadow-inner" placeholder="Masukkan base64 signature jika memverifikasi..."><?= htmlspecialchars($signatureInput) ?></textarea>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="flex flex-wrap gap-4 pt-6 border-t border-gray-800/80 bg-gray-800/10 p-6 -mx-6 md:-mx-8 -mb-6 md:-mb-8 mt-6">
                            <?php if ($activeTab === 'caesar' || $activeTab === 'xor'): ?>
                                <button type="submit" name="action" value="encrypt" class="px-8 py-3 bg-brand-600 hover:bg-brand-500 text-white text-sm font-bold tracking-wide uppercase rounded-lg transition-all"><i class="ph-bold ph-lock-key"></i> Encrypt</button>
                                <button type="submit" name="action" value="decrypt" class="px-8 py-3 bg-gray-800 border border-gray-700 hover:bg-gray-700 text-gray-200 text-sm font-bold tracking-wide uppercase rounded-lg transition-all"><i class="ph-bold ph-lock-key-open"></i> Decrypt</button>
                            <?php elseif ($activeTab === 'hash'): ?>
                                <button type="submit" name="action" value="hash" class="px-8 py-3 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold tracking-wide uppercase rounded-lg transition-all"><i class="ph-bold ph-fingerprint"></i> Compute Hash</button>
                            <?php elseif ($activeTab === 'rsa'): ?>
                                <button type="submit" name="action" value="encrypt" class="px-8 py-3 bg-brand-600 hover:bg-brand-500 text-white text-sm font-bold tracking-wide uppercase rounded-lg transition-all"><i class="ph-bold ph-shield-check"></i> Encrypt (Public)</button>
                                <button type="submit" name="action" value="decrypt" class="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold tracking-wide uppercase rounded-lg transition-all"><i class="ph-bold ph-shield-warning"></i> Decrypt (Private)</button>
                            <?php elseif ($activeTab === 'signature'): ?>
                                <button type="submit" name="action" value="sign" class="px-8 py-3 bg-pink-600 hover:bg-pink-500 text-white text-sm font-bold tracking-wide uppercase rounded-lg transition-all"><i class="ph-bold ph-pen-nib"></i> Sign Document</button>
                                <button type="submit" name="action" value="verify" class="px-8 py-3 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold tracking-wide uppercase rounded-lg transition-all"><i class="ph-bold ph-checks"></i> Verify Signature</button>
                            <?php endif; ?>
                        </div>
                    </form>
                </div>

                <?php if ($result && !isset($_POST['generate_keys']) && $activeTab !== 'signature' && $action !== 'verify'): ?>
                    <div class="mt-8 glass-card rounded-2xl shadow-2xl p-6 md:p-8 border-t border-t-gray-700/50 animate-[slideUpFade_0.3s_ease-out]">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-sm font-semibold tracking-wide text-gray-300 uppercase"><i class="ph-duotone ph-terminal-window text-brand-400"></i> Output Result</h3>
                            <button onclick="navigator.clipboard.writeText(document.getElementById('output-result').value)" class="text-[11px] uppercase font-bold bg-gray-800 text-brand-400 border border-brand-400/30 rounded py-1.5 px-3"><i class="ph-bold ph-copy"></i> Copy</button>
                        </div>
                        <textarea id="output-result" rows="5" readonly class="w-full bg-gray-900/80 border border-gray-700/80 rounded-xl p-5 text-brand-300 font-mono text-sm leading-relaxed outline-none resize-y custom-scrollbar shadow-inner"><?= htmlspecialchars($result) ?></textarea>
                    </div>
                <?php endif; ?>
